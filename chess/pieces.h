...
const int no_castling = 0;
const int king_side = 1;
const int queen_side = 2;
namespace piece {
...
public : 
	...
	piece():color(-1),line(0),column(0),status(0),prom('a'),move_count(0),indice(0),number(0) {}
...
public:
	bool valid_move(int newline , int newcolumn , piece*** board);
	king(int color, int line, int column, bool status, int move_count, int indice, int prom, int number) : piece(color, line, column, status, move_count, indice, prom, number) { prom = 'u'; castling = no_castling; };
